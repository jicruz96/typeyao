...
class Model(metaclass=ModelMeta):
    ...
    def __init_kwargs__(self, init_kwargs: dict[str, Any]) -> None:
        ...
        for kw, value in init_kwargs.items():
            ...
            try:
                ...
                assert field.init is True, f"'{field.name}' field has init=False"
    ...
    def __setattr__(self, __name: str, __value: Any) -> None:
        if __name in PROTECTED_MODEL_ATTRIBUTE_NAMES:
            raise AttributeError(f"{__name!r} is a protected attribute of {self.__class__.__name__}")
    ...
    def dict(self) -> dict[str, Any]:
        return {field_name: getattr(self, field_name) for field_name in self.__fields_map__.keys()}
